...
import { OPLRecord, Project, Workstation, User, DEPARTMENTS, STATUS_MAP } from '@/types';
...
const OPLPage: React.FC = () => {
  const { message } = App.useApp();
  const navigate = useNavigate();
  const { user } = useAppStore();
  const [form] = Form.useForm();
  const [loading, setLoading] = useState(false);
  const [data, setData] = useState<OPLRecord[]>([]);
  const [total, setTotal] = useState(0);
  const [pagination, setPagination] = useState({ current: 1, pageSize: 10 });
  const [modalVisible, setModalVisible] = useState(false);
  const [editingRecord, setEditingRecord] = useState<OPLRecord | null>(null);
  ...
  const fetchData = async (page = pagination.current, pageSize = pagination.pageSize) => {
    ...
    try {
      ...
      const res = await qmsApi.oplList(params);
  ...
  const handleEdit = (record: OPLRecord) => {
  ...
  const handleInitiateImprovement = async (id: number) => {
    try {
      const res = await improvementApi.fromOpl(id);
  ...
  const handleModalOk = async () => {
    try {
      ...
      if (editingRecord) {
        await qmsApi.oplUpdate(editingRecord.id, submitData);
        message.success('更新成功');
      } else {
        await qmsApi.oplCreate(submitData);
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 80,
      render: (flag: number, record: OPLRecord) => {
  ...
  return (
    <div>
      <Card>
        ...
        <ImportExportToolbar module="opl" onImportSuccess={() => fetchData(1, pagination.pageSize)} />
      ...
      <Modal title={editingRecord ? '编辑OPL' : '新增OPL'} open={modalVisible} onOk={handleModalOk} onCancel={() => setModalVisible(false)} destroyOnHidden
       className="modal-lg">
          ...
          <Row gutter={16}>
            ...
            <Col span={12}>
              <Form.Item name="handler_id" label="处理人">
                <FeishuUserSelect placeholder="请选择处理人" allowClear />
              </Form.Item>
            </Col>
          </Row>
